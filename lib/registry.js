...
Registry.prototype.publish = function(package) {
  ...
  return new Promise(function(resolve, reject) {
    exec(
      __dirname + '/../node_modules/.bin/npm publish --registry=' + _this.registryUrl() + '/' + _this.testRegistryName + '/_design/app/_rewrite',
...
Registry.prototype.unpublish = function(package) {
  ...
  return new Promise(function(resolve, reject) {
    exec(__dirname + '/../node_modules/.bin/npm unpublish ' + packageName + ' --registry=' + _this.registryUrl() + '/' + _this.testRegistryName + '/_design/app/_rewrite --force', function(err, stdout, stderr) {
      if (err) reject(err);
      else resolve(stdout.trim());
    });
  });
